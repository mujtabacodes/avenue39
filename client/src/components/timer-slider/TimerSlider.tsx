...
const TimerSlider: React.FC = () => {
  ...
  useEffect(() => {
    const updateTimers = () => {
      ...
      const newTimeRemaining: TTimeRemainingArray = timerSliderData.map((slide) => {
              ...
              <div className="bg-[#E3E4E6] lg:h-screen" key={index}>
                <div className="relative flex lg:flex-row flex-col justify-between h-full items-center lg:py-0 py-10">
                  <div className="w-full lg:px-28 px-5 text-left lg:flex lg:flex-col text-primary lg:order-1 order-2 lg:pt-0 pt-10">
                    <h3 className="text-black text-12 tracking-[4px] lg:mb-3">{slide.discountText}</h3>
                      ...
                      <div className="flex flex-col items-center">
                        <span className="lg:text-5xl text-lg font-light">
                          ...
                          <span className="lg:text-lg text-xs align-sub">
                            <span className="lg:text-3xl text-xs font-light opacity-50">/</span> Days
                          </span> :
                        </span>
                      </div>
                      <div className="flex flex-col items-center">
                        <span className="lg:text-5xl text-lg font-light">
                          {hours}
                          <span className="lg:text-lg text-xs align-sub">
                            <span className="lg:text-3xl text-xs font-light opacity-50">/</span> Hours
                          </span> :
                        </span>
                      </div>
                      <div className="flex flex-col items-center">
                        <span className="lg:text-5xl text-lg font-light">
                          {minutes}
                          <span className="lg:text-lg text-xs align-sub">
                            <span className="lg:text-3xl text-xs font-light opacity-50">/</span> Minutes
                          </span> :
                        </span>
                      </div>
                      <div className="flex flex-col items-center">
                        <span className="lg:text-5xl text-lg font-light">
                          {seconds}
                          <span className="lg:text-lg text-xs align-sub">
                            <span className="lg:text-3xl text-xs font-light opacity-50">/</span> Seconds
                          </span>
                        </span>
                      </div>
                    </div>
                    <Link href="/cart" className='bg-white py-2 px-3 rounded-full w-32 text-black hover:bg-black hover:text-white'>
                      {slide.buttonText}
                    </Link>
                  </div>
                  <div className="w-full lg:order-2 order-1 lg:p-16">
                    <Image src={slide.image} className="object-cover" alt="image" width={600} height={400} />
                  </div>
                </div>
              </div>
            );
