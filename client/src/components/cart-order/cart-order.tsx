...
const CartOrder: React.FC = () => {
  ...
  useEffect(() => {
    if (formik.values.country) {
      setFilteredCities(cities.filter(city => city.country === formik.values.country));
    } else {
      setFilteredCities([]);
    }
  ...
  return (
    <div className='shadow border border-[#EEEEEE] rounded-md p-2 md:p-4 w-full space-y-5 mt-5 md:mt-0'>
      ...
      <div className='flex justify-between items-center border-t-2 pt-4'>
        ...
        <p className='font-bold text-18'>Dhs.<span>450</span>.00</p>
      </div>
      <div className='flex flex-wrap md:flex-nowrap justify-between border-t-2 pt-4'>
        ...
        <form className='w-full md:w-6/12' onSubmit={formik.handleSubmit}>
          ...
          <Select onValueChange={(value) => formik.setFieldValue('city', value)}>
            <SelectTrigger className='rounded-none border'>
      ...
      <div className='flex justify-between items-center pt-4'>
        ...
        <p className='font-medium text-[26px]'>Dhs.<span>450</span>.00</p>
      </div>
      <div className='flex justify-between items-center border-t-2 pt-4'>
        <Button className='text-18 text-white w-full mt-2 h-[71px] rounded-sm'>Shop now</Button>
      </div>
    </div>
  )
}
